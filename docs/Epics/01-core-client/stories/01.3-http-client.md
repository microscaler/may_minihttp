...
- [x] `HttpClient::from_url(url: &str) -> io::Result<Self>` — connect an absolute HTTP/HTTPS URL
- [x] `HttpClient::from_url_with_tls_config(...)` — private CA, mTLS, and deterministic TLS tests
...
- [ ] URL parsing handles:
  ...
  - `https://host:port/path` (rustls; explicit port)
  - `https://host/path` (port defaults to 443)
  - URLs without a scheme are rejected; use `connect()` for raw host/port addresses
...
3. `HttpClient::from_url("https://example.com")` negotiates TLS and verifies the server identity
...
- [x] Unit tests: HTTP URL, HTTPS URL with local CA, Host header, and invalid scheme
...
- HTTPS uses rustls with the ring provider; default features are disabled so AWS-LC is not selected
- The default verifier uses operating-system trust; callers may inject a rustls configuration
